export const light = {
	bg: 'bg-gray-200',
	sidebarBg: 'bg-white',
	sidebarW: '56',
	sidebarBorder: 'border-r border-gray-200',
	sidebarItem: 'text-gray-600 rounded-md hover:text-gray-900 hover:bg-gray-50 focus:outline-none focus:bg-gray-100',
	sidebarItemActive: 'text-gray-900 rounded-md bg-gray-100 hover:text-gray-900 hover:bg-gray-100 focus:outline-none focus:bg-gray-200',
	headerBg: 'bg-white',
	headerShadow: 'shadow',
	contentBg: 'bg-gray-100',
	
}

export const dark = {
	bg: 'bg-gray-800',
	sidebarBg: 'bg-gray-800',
	sidebarW: '56',
	sidebarBorder: '',
	sidebarItem: 'text-gray-300 rounded-md hover:text-white hover:bg-gray-700 focus:outline-none focus:text-white focus:bg-gray-700',
	sidebarItemActive: 'bg-gray-900 focus:outline-none focus:bg-gray-700',
	headerBg: 'bg-gray-800',
	headerShadow: 'shadow',
	contentBg: 'bg-gray-100',
	
}

export const blue = {
	bg: 'bg-gray-200',
	sidebarBg: 'bg-indigo-800',
	sidebarW: '56',
	sidebarBorder: '',
	sidebarItem: 'text-indigo-300 rounded-md hover:text-white hover:bg-indigo-700 focus:outline-none focus:text-white focus:bg-indigo-700',
	sidebarItemActive: 'text-white rounded-md bg-indigo-900 focus:outline-none focus:bg-indigo-700',
	headerBg: 'bg-indigo-800',
	headerShadow: 'shadow',
	contentBg: 'bg-gray-100',
	
}

export const flat = {
	bg: 'custom-bg',
	sidebarBg: 'custom-bg',
	sidebarW: '56',
	sidebarBorder: '',
	sidebarItem: 'text-gray-400 group-hover:text-gray-600 group-focus:text-gray-300',
	sidebarItemActive: 'text-gray-500 rounded-md hover:text-gray-600 hover:bg-blue-200 focus:outline-none focus:text-gray-900 focus:bg-gray-100',
	headerBg: 'custom-bg',
	headerShadow: '',
	contentBg: 'custom-bg'
}	
