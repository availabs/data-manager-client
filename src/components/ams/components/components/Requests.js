...
export default ({ requests, ...props }) => {
  ...
  return !requests.length ? null : (
    <div className={ `
        mb-5 py-2 px-4 border-2 rounded ${ open ? theme.accent1 : "cursor-pointer" }
      ` }
      style={ { borderColor: "currentColor" } }
      onClick={ e => { stop(e); setOpen(true); } }>
      <div onClick={ e => { stop(e); setOpen(!open); } }
        className={ `border-b-2 cursor-pointer ${ open ? "border-current" : "border-transparent" }` }>
